...
async function main() {
  ...
  if (shouldWrite) {
    ...
    console.log(`State mutation baseline written (${sumCounts(current)} total).`);
    return;
  }
  ...
  console.log(`State mutation check passed (${sumCounts(current)} current, baseline ${baseline.total}).`);
}
