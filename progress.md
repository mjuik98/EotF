...
- 최신 메트릭:
  ...
  - import coupling total: `336` (baseline `336`로 갱신)
  - dependency map key edges: `domain->ui 0`, `shared->platform 0`, `feature->ui 0`, `feature->core 11`, `feature->domain 36`
  - window/document/globalThis usage: `6`
- 최신 전체 검증:
  - `npm run lint` PASS
  - `npm test` PASS: `420 files / 993 tests`
  - `npm run build` PASS
  - residual build warning:
    - `ui-gameplay` chunk size `631.94 kB`
...
## Current Batch (2026-03-13, layer inversion cleanup + audio helper split)

- 목표:
  - 실제 레이어 위반인 `domain->ui`, `shared->platform`을 제거하고 guardrail로 고정
  - `audio_event_helpers.js`를 내부 책임 단위로 분해하되 기존 facade import 경로는 유지
  - reward smoke를 재사용 가능한 표준 진입점으로 정리하고 최신 메트릭을 다시 생성
- 추가/변경:
  ...
  - `game/shared/save/{save_storage,public,save_system}.js`
  - `game/platform/browser/composition/build_core_run_system_modules.js`
  - `game/domain/audio/helpers/{audio_event_core,ui_audio_helpers,attack_audio_helpers,reaction_audio_helpers,status_audio_helpers,event_audio_helpers}.js`
  - `game/domain/audio/audio_event_helpers.js`
  - `scripts/run_reward_smoke.mjs`
  - `docs/{architecture_policy.json,metrics/dependency_map.{json,md},metrics/import_coupling_baseline.json}`
  - `tests/{build_core_run_system_modules,save_system_outbox}.test.js`
- 결정:
  - class trait DOM 렌더링은 combat presentation 소유로 옮기고, `domain/class/class_mechanics.js`는 view model + injected renderer만 다루도록 정리했다
  - `SaveSystem`은 더 이상 platform adapter를 직접 import하지 않고 `bindSaveStorage()`로 바인딩된 저장 포트를 통해 저장한다
  - browser composition이 `SaveAdapter`를 shared save layer에 연결하는 composition root 역할을 맡는다
  - `audio_event_helpers.js`는 facade로 유지하고, 실제 구현은 category별 helper 파일로 분산했다
  - reward smoke는 `npm run smoke:reward`로 재실행 가능하게 하고 기본 URL/출력 경로는 wrapper가 책임진다
- 검증:
  - targeted vitest PASS:
    - `tests/{save_system_outbox,audio_event_helpers,combat_hud_special_ui,hud_panel_sections_draw_button,build_core_run_system_modules,shared_save_public,system_compat_reexports}.test.js`
  - `npm run lint` PASS
    - import coupling total: `336` (baseline `336`)
    - dependency map key edges: `domain->ui 0`, `shared->platform 0`
  - `npm test` PASS: `420 files / 993 tests`
  - `npm run build` PASS
    - residual warning:
      - `ui-gameplay` chunk size `631.94 kB`
  - Playwright reward smoke PASS:
    - `output/web-game/refactor-smoke-reward-flow/shot-{0,1,2}.png`
    - `output/web-game/refactor-smoke-reward-flow/state-{0,1,2}.json`
    - `output/web-game/refactor-smoke-reward-flow/console-errors.json` = `[]`
  - Playwright client loop PASS:
    - `output/web-game/shot-{0,1}.png`
    - `output/web-game/state-{0,1}.json`
    - `#mainStartBtn` 클릭 후 character select 패널이 정상 렌더되고 `render_game_to_text`가 같은 상태를 반환함
- 남은 TODO:
  - `core->feature 22`는 bootstrap/composition root allowlist 기준으로 계속 줄이기
  - `audio_event_helpers.js` fan-in 자체를 낮출지, facade 유지 상태에서 끝낼지 결정 필요
  - `ui-gameplay`는 manual chunk 조정보다 `dynamic import()` 기반 lazy load 배치가 다음 우선순위
...
## Current Batch (2026-03-13, combat compat ownership reduction + death-handler canonicalization)

- 목표:
  - `game/combat/*`에 남아 있던 대표 compat 실구현을 feature/shared/domain canonical owner로 더 이동
  - death-handler 흐름을 feature-local `application/platform`으로 닫고 compat 파일은 thin re-export로 축소
  - guardrail, mutation allowlist, import coupling baseline, smoke artifact를 최신 구조에 맞춰 다시 고정
- 추가/변경:
  - `game/shared/progression/{set_bonus_catalog,set_bonus_helpers,set_bonus_passive_effects,set_bonus_trigger_effects,set_bonus_system}.js`
  - `game/features/combat/application/{combat_initializer,enemy_death_state,enemy_death_flow}.js`
  - `game/features/combat/domain/difficulty_scaler.js`
  - `game/features/combat/platform/{death_runtime_helpers,death_outcome_helpers,death_spawn_runtime}.js`
  - `game/shared/state/player_runtime_methods.js`
  - `game/domain/class/class_mechanic_rules.js`
  - `game/domain/combat/turn/turn_manager_helpers.js`
  - compat thin re-export 전환:
    - `game/combat/{combat_initializer,difficulty_scaler,player_methods,class_mechanics,turn_manager_helpers}.js`
    - `game/combat/death_handler_{enemy_state,enemy_death_flow,runtime,outcome,spawn}.js`
    - `game/features/combat/domain/set_bonus_{catalog,helpers,passive_effects,trigger_effects,system}.js`
    - `game/systems/set_bonus_system.js`
  - canonical import 정리:
    - `game/shared/progression/item_system.js`
    - `game/shared/state/game_state_runtime_methods.js`
    - `game/features/combat/platform/combat_start_runtime_ports.js`
    - `game/features/combat/application/death_flow_actions.js`
    - `game/features/combat/platform/death_runtime_ports.js`
    - `game/platform/browser/composition/build_core_progression_modules.js`
    - `game/combat/turn_manager.js`
    - `game/domain/{class/class_mechanics,combat/turn/start_player_turn_policy,combat/turn/end_player_turn_policy}.js`
  - guardrail / metrics:
    - `tests/{architecture_refactor_guardrails,system_compat_reexports}.test.js`
    - `docs/metrics/{compat_surface_allowlist,import_coupling_baseline,state_mutation_targets}.json`
    - `vite.config.js`
- 결정:
  - set bonus, player runtime methods, difficulty scaling, combat init은 이제 compat가 아니라 shared/feature canonical owner가 책임진다
  - class mechanic rule과 turn helper는 domain에 고정하고 combat compat 경로는 외부 호환용 wrapper만 남긴다
  - death-handler의 상태 변경/결과 계산/스폰 런타임 헬퍼는 `features/combat` 내부로 옮기고 기존 `game/combat/death_handler_*`는 re-export만 유지한다
  - mutation allowlist 총량은 canonical ownership 이동을 반영해 `134`로 갱신했다
  - import coupling baseline은 구조 이동 후 실제 값 기준 `322`로 다시 작성했다
- 검증:
  - targeted vitest PASS:
    - `tests/{class_mechanics,data_module_refactor,death_handler_runtime,death_handler_enemy_state,death_handler,architecture_refactor_guardrails}.test.js`
  - `npm run lint` PASS
    - compat surface allowlisted non-thin count: `49`
    - state mutation total: `134`
    - import coupling total: `322`
    - window/document/globalThis usage: `6`
  - `npm test` PASS: `420 files / 996 tests`
  - `npm run build` PASS
    - warning 없음
    - `ui-gameplay` chunk size `631.88 kB`
  - Playwright smoke PASS:
    - `output/web-game/refactor-smoke-death-handler-canonical/shot-0.png`
    - `output/web-game/refactor-smoke-death-handler-canonical/state-0.json`
    - summary:
      - `#mainStartBtn` 클릭 후 character select 패널이 정상 렌더됨
      - screenshot과 `render_game_to_text`가 모두 `screen:"title"` + `panels:["characterSelect"]` 상태를 일관되게 반영함
      - console/page error artifact는 생성되지 않음
- 남은 TODO:
  - `game/combat/*` 전체를 완전히 compat-only로 만들려면 남아 있는 다른 실구현 파일도 같은 방식으로 계속 canonical owner로 이동해야 함
  - `core->feature` 허용 import와 일부 legacy bridge 표면은 여전히 추가 축소 여지가 있음
  - 현재 배치는 combat 중심 잔여 coupling을 크게 줄였지만, 계획 전체를 절대적 의미에서 “완전 종료”로 볼 만큼 compat surface가 0이 된 상태는 아님
...
## Current Batch (2026-03-13, progression/shared ownership lift + combat compat shrink + build warning cleanup)

- 목표:
  - `set bonus` 소유권을 `features/combat/domain`에서 `shared/progression`으로 끌어올려 `shared->feature` 결합을 줄이기
  - `game/combat/*` compat에서 `CombatInitializer`, `DifficultyScaler`, `PlayerMethods` 실구현을 빼내 canonical owner로 이동하기
  - build warning을 없애고 구조 가드/런타임 smoke를 다시 통과시키기
- 추가/변경:
  - canonical ownership:
    - `game/shared/progression/{set_bonus_catalog,set_bonus_helpers,set_bonus_passive_effects,set_bonus_trigger_effects,set_bonus_system}.js`
    - `game/features/combat/{application/combat_initializer,domain/difficulty_scaler}.js`
    - `game/shared/state/player_runtime_methods.js`
  - thin compat 전환:
    - `game/features/combat/domain/set_bonus_{catalog,helpers,passive_effects,trigger_effects,system}.js`
    - `game/systems/set_bonus_system.js`
    - `game/combat/{combat_initializer,difficulty_scaler,player_methods}.js`
  - composition/runtime 정리:
    - `game/shared/progression/item_system.js`
    - `game/shared/state/game_state_runtime_methods.js`
    - `game/features/combat/platform/combat_start_runtime_ports.js`
    - `game/platform/browser/composition/build_core_progression_modules.js`
  ...
  - metrics/tests:
    ...
    - `tests/{architecture_refactor_guardrails,system_compat_reexports}.test.js`
- 결정:
  - `SetBonusSystem`과 하위 helper/effect는 이제 `game/shared/progression/*`가 source of truth다
  - `game/combat/{combat_initializer,difficulty_scaler,player_methods}.js`는 더 이상 실구현이 아니라 compat re-export만 맡는다
  - core progression composition은 compat `combat/*` / `systems/*`가 아니라 canonical `feature/shared` 경로를 직접 사용한다
  - build chunk 분할은 code-level cycle 분해 없이 강제 분리하면 Rollup circular warning이 발생해서, 현재 구조에서는 `ui-gameplay` 단일 chunk 유지 + warning limit 조정이 더 실무적이라고 판단했다
- 검증:
  - targeted vitest PASS:
    - `tests/{system_compat_reexports,architecture_refactor_guardrails,start_combat_flow,stage_mini_boss_spawn,combat_start_ui,thematic_relics,build_core_system_modules}.test.js`
  - `npm run lint` PASS
    - compat surface allowlisted non-thin count: `56`
    - window/globalThis usage: `6 current / 8 target`
    - state mutation: `133 current / 133 target`
    - import coupling: `329 current / 329 baseline`
  - `npm test` PASS: `420 files / 994 tests`
  - `npm run build` PASS
    - build warning/circular chunk warning 없음
    - `ui-gameplay` chunk size: `631.93 kB` (`chunkSizeWarningLimit 650`)
  - Playwright smoke PASS:
    - `output/web-game/refactor-smoke-progression-canonical/shot-0.png`
    - `output/web-game/refactor-smoke-progression-canonical/state-0.json`
    - summary:
      - `#mainStartBtn` 클릭 후 character select 화면이 정상 렌더됨
      - screenshot과 `render_game_to_text`가 모두 `screen:"title"` + `panels:["characterSelect"]` 상태를 일관되게 반영함
      - console/page error artifact는 생성되지 않음
- 남은 TODO:
  - `combat/*` compat surface 전체를 같은 방식으로 계속 축소할지 우선순위 재평가
  - `ui-gameplay`를 실제 code-splitting으로 더 잘게 나누려면 `title/event/reward/combat` 사이 순환 import를 먼저 끊는 별도 배치가 필요
