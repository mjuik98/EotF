...
- 핵심 메트릭:
  ...
  - `vitest`: `261 files / 653 tests` PASS
...
- 신규 회귀 테스트:
  ...
  - 직접 global fallback 제거에 맞춘 `tests/death_handler.test.js`, `tests/player_hp_panel_ui.test.js`, `tests/combat_start_ui.test.js` 갱신
- 최근 회귀 수정:
...
- 현재 게이트 기준선은 `window/document/globalThis usage: 44`, `import coupling: 141`, `vitest: 264 files / 658 tests`다.

## Next Priorities

1. binding/orchestration helper 중 아직 큰 fan-out을 가진 파일을 추가 분해한다.
2. 남은 `window/globalThis` 직접 접근, 현재는 `game/ui/cards/card_ui.js`, `game/ui/cards/card_target_ui.js` 같은 카드 runtime UI 경로를 feature port 경계 뒤로 더 밀어 넣는다.
