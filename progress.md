...
- 최신 메트릭:
  ...
  - import coupling total: `408`
  - window/document/globalThis usage: `8`
- 최신 전체 검증:
  - `npm run lint` PASS
  - `npm test` PASS: `374 files / 888 tests`
  - `npm run build` PASS
  - Playwright smoke PASS:
    - `output/web-game/arch-refactor-public-facade-batch-37/shot-0.png`
    - `output/web-game/arch-refactor-public-facade-batch-37/shot-2.png`
    - `output/web-game/arch-refactor-public-facade-batch-37/state-0.json`
    - `output/web-game/arch-refactor-public-facade-batch-37/state-2.json`
...
1. `platform/legacy`의 남은 adapter/query surface가 feature 내부 `app/ports`나 `ui` 세부 구현을 직접 보는 지점을 찾아 `features/*/public.js`로 올린다.
2. `core/system`과 `core/deps`에 남아 있는 비-feature service 의존을 검토해서 `shared` 또는 feature facade로 수렴할 수 있는 후보를 추린다.
3. `runtime_state_use_case`에 남아 있는 compat import를 더 줄이고 `game/shared/state/*`를 공용 상태 경계로 굳힌다.
4. 신규 아키텍처 가드의 `sourceFiles + denyTargetPrefixes` 규칙을 legacy/core의 남은 조립 지점까지 점진 확장한다.
