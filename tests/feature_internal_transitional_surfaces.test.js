...
describe('feature internal transitional surfaces', () => {
  it('keeps selected transitional files as thin re-exports to canonical owners', () => {
    const expectations = {
      ...
      'game/features/event/app/reward_actions.js': /export\s+\{\s*createRewardActions\s*\}\s+from\s+'\.\.\/integration\/reward_runtime_capabilities\.js';/,
      'game/features/event/app/reward_navigation_actions.js': /export\s+\{\s*createRewardNavigationActions\s*\}\s+from\s+'\.\.\/integration\/reward_runtime_capabilities\.js';/,
