...
const hoisted = vi.hoisted(() => ({
  ...
  applyPlayerMaxEnergyGrowthCompatState: vi.fn((gs, amount) => {
  ...
  applyPlayerMaxHpGrowthCompatState: vi.fn((gs, amount) => {
...
describe('event/reward state command ownership', () => {
  ...
  it('routes event shop player mutations through shared player state commands', async () => {
    ...
    expect(hoisted.applyPlayerGoldCompatState).toHaveBeenCalledWith(state, -20);
    expect(hoisted.applyPlayerGoldCompatState).toHaveBeenCalledWith(state, -15);
    expect(hoisted.applyPlayerGoldCompatState).toHaveBeenCalledWith(state, -12);
    expect(hoisted.applyPlayerMaxEnergyGrowthCompatState).toHaveBeenCalledWith(state, 1, { maxEnergyCap: 5 });
