...
export function saveMetaForSystem(system, deps = {}) {
  ...
  try {
    ...
    const persisted = system._persistWithOutbox(metaKey, meta);
    if (!persisted) {
      system._lastSaveError = new Error(`[SaveSystem] meta ${SAVE_ERROR_QUEUED}`);
      return { status: 'queued', persisted: false, queueDepth: system.getOutboxSize() };
    }
    return { status: 'saved', persisted: true, queueDepth: system.getOutboxSize() };
  } catch (error) {
    Logger.error('[SaveSystem] Meta save failed:', error?.name, error?.message);
    system._lastSaveError = error;
    return { status: 'error', persisted: false, queueDepth: system.getOutboxSize(), error };
  }
}
...
export function saveRunForSystem(system, deps = {}) {
  const gs = getGS(deps);
  if (!gs?.player) return { status: 'skipped', persisted: false, reason: 'missing-player', queueDepth: system.getOutboxSize() };
  ...
  if (!isGameStarted) return { status: 'skipped', persisted: false, reason: 'game-not-started', queueDepth: system.getOutboxSize() };
  if (gs.combat?.active) return { status: 'skipped', persisted: false, reason: 'combat-active', queueDepth: system.getOutboxSize() };

  try {
    const save = buildRunSave(gs, RUN_SAVE_VERSION);
    const persisted = system._persistWithOutbox(saveKey, save);
    if (!persisted) {
...
export function readRunSaveRecordForSystem(system, { logErrors = true, ...deps } = {}) {
  ...
  return readRunSaveRecord({
    ...
    dropOutboxKey: (key) => system._dropOutboxKey(key),
