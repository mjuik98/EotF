import { resolvePlayerRuntimeEffectCompat } from '../../platform/legacy/player/player_runtime_effect_compat.js';
...
export const PlayerRuntimeEffectMethods = {
  ...
  heal(amount, source = null, deps = {}) {
    ...
    if (this.combat?.active) {
      const compat = resolvePlayerRuntimeEffectCompat(deps);
      const cm = compat.classMechanics?.[this.player.class];
  ...
  addSilence(amount, label = '소음', deps = {}) {
    ...
    if (silenceGauge >= max) {
      ...
      if (compat.screenShake) compat.screenShake.shake(10, 0.5);
    }
    if (typeof compat.updateNoiseWidget === 'function') compat.updateNoiseWidget();
    if (typeof compat.updateClassSpecialUI === 'function') compat.updateClassSpecialUI();
  },
  ...
  addTimeRift(amount, label = '시간의 균열', deps = {}) {
    ...
    if (timeRiftGauge >= max) {
      ...
      if (compat.screenShake) compat.screenShake.shake(15, 0.6);

      compat.setTimeoutFn(() => {
        if (typeof compat.endPlayerTurn === 'function') {
          compat.endPlayerTurn();
        }
      }, 300);
    }
  },
