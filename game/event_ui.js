...
function _renderChoices(event, doc) {
  ...
  choicesEl.innerHTML = event.choices.map((choice, idx) => `
      <div class="event-choice" onclick="resolveEvent(${idx})">${SecurityUtils.escapeHtml(choice.text)}</div>
    `).join('');
}
  ...
  showCardDiscard(gsArg, isBurn = false, deps = {}) {
    ...
    overlay.innerHTML = `
        <div style="text-align:center;">
          ...
          <div style="font-family:'Cinzel Decorative',serif;font-size:22px;font-weight:900;color:var(--white);margin-bottom:6px;">${SecurityUtils.escapeHtml(title)}</div>
          <div style="font-family:'Crimson Pro',serif;font-style:italic;font-size:13px;color:var(--text-dim);">${SecurityUtils.escapeHtml(sub)}</div>
        </div>
    ...
    uniqueCards.forEach(cardId => {
      ...
      btn.innerHTML = `
          ...
          <div style="font-family:'Cinzel',serif;font-size:10px;font-weight:700;color:${rarityColor[card.rarity] || 'var(--white)'};margin-bottom:3px;">${SecurityUtils.escapeHtml(card.name)}</div>
          <div style="font-size:10px;color:var(--text-dim);line-height:1.3;">${SecurityUtils.escapeHtml(card.desc || '')}</div>
  ...
  showItemShop(gsArg, deps = {}) {
    ...
    shopItems.forEach(item => {
      ...
      card.innerHTML = `
          ...
          <div style="font-family:'Cinzel',serif;font-size:16px;font-weight:700;color:${rc.color};margin-bottom:6px;">${SecurityUtils.escapeHtml(item.name)}</div>
          <div style="font-size:13px;color:var(--text-dim);line-height:1.4;margin-bottom:10px;flex:1;">${window.DescriptionUtils ? window.DescriptionUtils.highlight(SecurityUtils.escapeHtml(item.desc)) : SecurityUtils.escapeHtml(item.desc)}</div>
