import {
  ...
  resolveCodexCardId,
} from '../domain/codex_card_variants.js';
...
function ensureMetaObject(gs) {
  const meta = gs?.meta;
  if (meta && typeof meta === 'object' && !Array.isArray(meta)) return meta;
  Object.assign(gs, { meta: {} });
  return gs.meta;
}
...
export function ensureCodexState(gs) {
  ...
  const meta = ensureMetaObject(gs);
  if (!meta.codex || typeof meta.codex !== 'object' || Array.isArray(meta.codex)) {
    Object.assign(meta, { codex: {} });
  }
...
export function ensureCodexRecords(gs) {
  ...
  if (!records || typeof records !== 'object' || Array.isArray(records)) {
    Object.assign(meta, {
      codexRecords: { enemies: {}, cards: {}, items: {} },
    });
  } else {
...
export function registerCardDiscovered(gs, cardId, options = {}) {
  ...
  if (!entry) return null;
  if (isCardUpgradeVariant(rawKey)) {
