...
function resolveLiveCharge(itemId, gs) {
  const chargeMeta = ITEM_CHARGE_META[itemId] || null;
  if (!chargeMeta || !gs) return null;
  const val = gs[chargeMeta.gsKey];
  if (chargeMeta.type === 'bool') return { type: 'bool', active: !!val, label: chargeMeta.label };
  if (chargeMeta.type === 'invert-dot') {
    const used = val ? 1 : 0;
    return { type: 'dot', val: chargeMeta.max - used, max: chargeMeta.max, remaining: chargeMeta.max - used, label: chargeMeta.label };
  }
  const cur = Number(val) || 0;
  return { type: chargeMeta.type, val: cur, max: chargeMeta.max, remaining: chargeMeta.max - cur, label: chargeMeta.label };
}

function resolveSetInfo(item, data, gs, setBonusSystem) {
  let setDef = null;
  let setCount = 0;
  let setOwnedFlags = [];
  ...
  return { setDef, setCount, setOwnedFlags };
}

function removeItemTooltipElement(win) {
  if (!win.__itemTooltipEl) return;
...
export function showItemTooltipUi(event, itemId, deps = {}) {
  ...
  const liveCharge = resolveLiveCharge(itemId, gs);
  const { setDef, setCount, setOwnedFlags } = resolveSetInfo(item, data, gs, setBonusSystem);
  ...
  rarityBadge.style.cssText = `font-family:'Cinzel',serif;font-size:8px;letter-spacing:0.15em;background:rgba(${ra.rgb},0.1);border:1px solid rgba(${ra.rgb},0.35);border-radius:4px;padding:2px 7px;color:${ra.color};`;
