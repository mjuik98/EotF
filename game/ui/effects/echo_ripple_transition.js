...
export function startEchoRippleDissolve(overlayEl, deps = {}) {
  ...
  function draw(now) {
    ...
    scatterParticles.forEach((p) => {
      ...
      _ctx.fillStyle = `rgba(${r},${g},${b},${p.life * 0.6})`;
      _ctx.fill();
    });

    if (elapsed < 150) {
      const flashA = (1 - elapsed / 150) * 0.45;
      const flashR = Math.max(0, elapsed * 2.5);
      if (flashR > 0) {
        const flash = _ctx.createRadialGradient(cx, cy, 0, cx, cy, flashR);
        flash.addColorStop(0, `rgba(220,180,255,${flashA})`);
        flash.addColorStop(0.4, `rgba(168,85,247,${flashA * 0.6})`);
        flash.addColorStop(1, 'rgba(123,47,255,0)');
        _ctx.beginPath();
        _ctx.arc(cx, cy, flashR, 0, Math.PI * 2);
        _ctx.fillStyle = flash;
        _ctx.fill();
      }
    }

    const allWavesDone = WAVES.every((wave) => {
      const t = elapsed - wave.delay;
      return t > 0 && Math.min(1, t / WAVE_DURATION_MS) >= 0.99;
    });

    if (allWavesDone && overlayAlpha <= 0 && !dissolveComplete) {
      ...
      setTimeout(() => {
        ...
        if (_blackOutEl) {
          _blackOutEl.style.transition = 'opacity 0.26s ease';
