...
const ENEMY_EFFECTS = {
  ...
  heal_12: (gs, enemy) => {
    ...
    gs.addLog(`💚 ${enemy.name}: 체력 회복 (+${heal})`, 'heal');
  },
  heal_15: (gs, enemy) => {
    const heal = 15;
    enemy.hp = Math.min(enemy.maxHp, enemy.hp + heal);
...
export const CombatTurnUI = {
  ...
  async enemyTurn(deps = {}) {
    ...
    if (gs.runConfig?.curse === 'fatigue' || gs.meta?.runConfig?.curse === 'fatigue') {
      // 이미 0으로 초기화했으므로 추가 행동 필요 없음 (방어막 획득 시 감쇄는 여기서 처리하지 않음)
      // 다만 계획에 'max shield -10'이 있으므로 방어막 추가 시점에 적용하는 것이 맞을 수 있음.
      // 여기서는 일단 턴 시작 시 로그로 알림
    }
